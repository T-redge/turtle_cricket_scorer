        ...
        if !self.return_hide_player_list() {
            ...
            if !self.innings.return_innings_finished() {
                if !self.player_select.return_hide_player_selector() {
                    let bat_team = match self.home_team.return_team_role() {
                        TeamRoles::Bat => {&self.home_team.return_team_name()}
                        TeamRoles::Bowl => {&self.away_team.return_team_name()}
                        TeamRoles::Waiting => {""}
                    };
                    create_player_selector(ctx, self, "Select opening batters!", bat_team, 2);
                } else {
    ...
    egui::SidePanel::left("id_scorecard")
        ...
        .show(ctx, |ui| {
            ...
            ui.horizontal_wrapped(|ui| {
                ...
                ui.end_row();
                ui.label(create_game_label(&game.bat_1.return_profile(), 16.0))
            });
        });
}
...
fn create_player_selector(ctx: &egui::Context, game: &mut Game, selection_text: &str, team_name: &str, num_players: u8) {
    egui::Window::new(selection_text)
        ...
        .show(ctx, |ui| {
            ...
            game.player_select.show_player_selector(ui,team_name, num_players);
            let player = game.player_select.return_chosen_player();
